# Axigen CLI Reference (10.5.0)

> Auto-generated from Axigen server cliHelp dump. Do not edit manually.
> To refresh, use: `python3 axigen_cli.py --dump-help` and reconvert.
...
*The commands available for the RPOP context are:*
...
WARNING
The migrate command, when the parameter overrideQuota is set to its
"no" default, will migrate mails until the mailbox Quota is reached.
If overrideQuota is set to yes, all the mails will be migrated but the mailbox
Quota could be exceeded, in which case, the account will no longer receive any more mails
...
REPAIR Accounts <accountName> [<accountName>...] - try to recover lost accounts
...
## Context: subdomain-create

*The commands available for the Subdomain-Create context are:*
...
*The commands available for the Domain-Edit context are:*
...
SET [twoFactorKey <string>] - sets the two factor authentication secret key
...
SHOW TotpToken - shows current TOTP token of the current account
...
*The commands available for the Group context are:*
...
*The commands available for the Folder Recipient context are:*
...
*The commands available for the Public Folders context are:*
...
## Context: webmaildata (as subcontext of account)
...
## Context: webmaildata (as subcontext of list)
...
## Context: limits (as subcontext of account)
...
## Context: limits (as subcontext of list)
...
## Context: filters (as subcontext of account|accountClass)
...
## Context: filters (as subcontext of server|domain|folderRcpt|group|list)
...
ADD Preset [name] <name> - adds a filter preset to the domain (changes context)
UPDATE Preset [name] <name> - updates a filter preset from the domain (changes context)
...
SET [filterName <name>] - sets the name of the filter as defined in the socket/script object sets
SET [filterType <type>] - sets type of the filter (to which object set belongs)
SET [filteringOn <type>] - applies filter on levels: received|sent|sentAndReceived
...
## Context: listener (as subcontext of proxy services)
...
## Context: listener (as subcontext of non proxy services)
...
*The commands available for the Service Access Control context are:*
...
CREATE Domain name <name> domainLocation <path> wmFilterTemplatePath <sieve file path> postmasterPassword <pass> - creates a domain (changes context)
...
EDIT Domain wmFilterTemplatePath <sieve file path> - edits a domain from the server using the wmfilter template sieve file (changes context)
...
RESTORE Domain name <domainName> - restores a domain (changes context)
...
SET premiumAccountAddons name <name> addons (list of addons) - sets the addons for a premium account
        Possible addons: outlook|activesync|sharing|personalorganizer|ia

LIST premiumFolderRcpts [domain <name>] [wildcard (ex: account*)] - lists the "premium" folder recipients (all or per domain)
SET premiumFolderRcptsAddons name <name> addons (list of addons) - sets the addons for a "premium" folder recipient
        Possible addons: ia
...
SET [module <module>] - sets module of the user of this rule.
        valid names: SMTP-IN SMTP-OUT IMAP POP3 WEBMAIL WEBADMIN DNR RPOP CLI PROCESSING LOG
                     REPORTING FTP-BACKUP PROXY_POP3 PROXY_IMAP USERDB PROXY_WEBMAIL FILTERS
                     CALLOG SECURITY JOBLOG SERVER(SRVLOG) MAILBOXAPI
                     * = ALL)
...
## Context: ldap-connector

*The commands available for the Ldap Connector context are:*
...
*The commands available for the ConnectionData context are:*
...
*The commands available for the MappingData context are:*
...
*The commands available for the UserMap context are:*
...
SET [enableSRS <yes|no>] - sets the sender rewriting scheme secret key
SET [srsSecretKey <key>] - enables/disables sender rewriting scheme
...
*The commands available for the IMAP Proxy context are:*
...
## Context: webmailProxy

*The commands available for the Webmail Proxy context are:*
...
*The commands available for the User Database context are:*
...
ADD ldapConnector [name] <name> ldapUri <uri> - adds a ldap connector to the service (changes context)
UPDATE ldapConnector [name] <name> - updates a ldap connector from the service (changes context)
...
LIST FILTER <filters> - lists the result of the filters applied on queue entries
  where <filters> can be a subset separated by spaces of the following: - $
...
*The commands available for the Webdav tracer context are:*
...
*The commands available for the Administrative ACL context are:*
...
*The commands available for the Group Administrative ACL context are:*
...
SHOW effective-permissions [domain <name>] - shows group's effective permissions on server or a domain

List of possible server permission names
    permissions:  - <list_of_permissions>

List of possible domain permission names
    permissions:  - <list_of_permissions>

## Context: adminUser

*The commands available for the User Administrative ACL context are:*
...
SHOW effective-permissions [domain <name>] - shows user's effective permissions on server or a domain
...
*The commands available for the Resource permissions context are:*
...
DENY permission (<permNames>) admin-group [name] <name> - denies permissions to group

List of possible permission names
    permissions:  - <list_of_permissions>
...
The following parameters' value should be a RFC2822 compliant mail
...
ADD User [email] <email> name <name> - adds an user to the list (changes context)
...
## Context: limits (as subcontext of account|accountClass)
...
SET [enableTwoFactorAuth <disabled|enabled|mandatory>] - enables/disables two factor authentication
...
SET [passwordRecoveryMethods ([secretQuestion] [peaBased])] - sets the methods available for password recovery
SET [usernameRecoveryMethods ([peaBased])] - sets the methods available for user name recovery
SET [passwordRecoveryFileName <path>] - name of the file containing template email message sent to user's PEA for password recovery
SET [usernameRecoveryFileName <path>] - name of the file containing template email message sent to user's PEA for username recovery
...
## Context: limits (as subcontext of domain)
...
## Context: quotas (as subcontext of account)
...
## Context: quotas (as subcontext of accountClass)
...
## Context: quotas (as subcontext of domain)
...
## Context: sendRecvRestrictions (as subcontext of account|accountClass)

*The commands available for the Send/Recv Restrictions context are:*
...
## Context: sendRecvRestrictions (as subcontext of domain)
